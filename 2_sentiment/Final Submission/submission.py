...
def kmeans(examples, K, maxIters):
    ...
    random.seed(42)
    centroids = [random.choice(examples) for k in range(K)]
    
    loss_example_norm = 0
    num_iters = 0
    assignments = ["" for elem in examples]
    example_norms = [0 for elem in examples]
    centroid_norms = [0 for elem in range(K)]
    
    # LOSS - Example Norm
    for i in range(len(examples)):
        example_norms[i] = dotProduct(examples[i], examples[i])
    ...
    while num_iters < maxIters:
        ...
        for i in range(len(examples)):
            ...
            for j in range(len(centroids)):
                # LOSS - Cluster Norm
                centroid_norms[j] = dotProduct(centroids[j], centroids[j])
                distances.append(example_norms[i] + centroid_norms[j] - 2*dotProduct(examples[i], centroids[j]))
        ...
        # Re-calculate centroids based on avg coordinates of assigned examples
    ...
    # Compute the final reconstruction cost
    reconstruction_loss = 0
    for i in range(len(example_norms)):
        j = assignments[i]
        reconstruction_loss += example_norms[i] + centroid_norms[j] - 2*dotProduct(examples[i], centroids[j])
