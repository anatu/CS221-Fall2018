    ...
    for iteration in range(numIters):
        ...
        for example in trainExamples:
            ...
            for feat in features.keys(): 
                if feat in weights: weights[feat] = 0
            ...
            # Compute gradient vector based on value of 
            hingeGrad = map(lambda z: -1*z*y, features.values()) if (score_product < 1) else 0
            
            # Update only if the gradient is nonzero, otherwise 
            # gradient descent cannot proceed
            if hingeGrad != 0:
                for feature in weights.keys():
                    weights[feature] = weights[feature] - map(lambda z: eta*z, hingeGrad.values())
