...
def learnPredictor(trainExamples, testExamples, featureExtractor, numIters, eta):
    ...
    for iteration in range(numIters):
        ...
        # Select a SGD example from the training set
        example = random.choice(trainExamples)
        ...
        score_product = (sum(weights.get(key, 0)*features.get(key, 0) for key in set(weights) & set(features)))*y
...
#                 loss_cluster_norm += sum((centroids[j].get(d))**2 for d in centroids[j])
        ...
        for k in range(len(centroids)):
            ...
            for assignee in assignees:
                ...
                reconstruction_loss += sum(recr_loss)
                # LOSS - Cross Term
#                 loss_cross_term += -2*sum(centroids[k].get(key,0)*assignee.get(key,0) for key in set(centroids[k]) & set(assignee))
