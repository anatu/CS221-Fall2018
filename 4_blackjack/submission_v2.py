...
class BlackjackMDP(util.MDP):
    ...
    def succAndProbReward(self, state, action):
        ...
        # Boolean return checker for end states
        def isEnd(state, action):
            # If the player quits - reward is what they have in hand
            if action == "Quit":
                reward = state[0]
                return True
            # Goes bust - total value is over threshold
            elif state[0] > self.threshold:
                reward = 0
                return True
            # Deck runs out of cards, but not from player drawing the last card
            elif sum(state[2]) == 0:
                reward = state[0]
                return True
            # Deck runs out of cards from player drawing the last card
            elif (sum(state[2]) == 1) & (action == "Take"):
                # Reward is either sum of values, or they go bust
                reward = state[0] if state[0] < self.threshold else 0
                return True
            else:
                return False

        # Check if end state is reached
        if isEnd(state, action) == True:
            return []
        ...
        if action == "Take":
            ...
            for i in range(len(ft_counts)):
                if ft_counts[i] > 0:
                    ...
                    probability = float(ft_counts[i])/ft_sum
                    # Decrement by 1 the amount of the chosen card being taken
                    new_ft = list(ft_counts)
                    new_ft[i] = new_ft[i] - 1
                    new_ft = tuple(new_ft)
                    # Create the new state
                    # If the score is higher than the threshold, the player will go bust
                    # in the next iteration when isEnd is called
                    newState = (state[0] + self.cardValues[i], None, new_ft)
