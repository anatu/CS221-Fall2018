...
class QLearningAlgorithm(util.RLAlgorithm):
    ...
    def incorporateFeedback(self, state, action, reward, newState):
        ...
        for feature in features:
            key = feature[0]
            value = feature[1]
            self.weights[key] = self.weights.get(key, 0) - stepSize*(Q_current - (reward + self.discount*V_opt))
...
def blackjackFeatureExtractor(state, action):
    ...
    if counts != None:
        count_tuple = [0 for elem in counts]
        for i in range(len(counts)):
            if counts[i] != 0:
                count_tuple[i] = 1
            else:
                count_tuple[i] = 0
        features.append(((action, tuple(count_tuple)), 1))

        # Indicators on the action and number of cards remaining 
        for i in range(len(counts)):
            features.append(((action, counts[i]), 1))
