    ...
    def succAndProbReward(self, state, action):
        ...
        def isEnd(state, action):
        	...
        	# Goes bust
				...
				reward = 0
        		return True
        	# Deck run out of cards, not drawing last card
			elif sum(state[2]) == 0:
				reward = state[0]
				return True
			elif sum(state[2]) == 1:
    	...
    	for i in ft_counts:
    		if ft_counts[i] > 0:
                # Calculate denominator of prob. of occurrence
        ...
        # PEEK ACTION
        if action == "Peek":
        	# Check if peek has already been done
        	if state[1] != None:
        		return []
        	# Otherwise update with possible peek results
        		...
        		for i in ft_counts:
        			if ft_counts[i] > 0:
		    			probability = ft_counts[i]/sum(ft_counts)
