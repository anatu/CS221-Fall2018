        ...
        if action == "Take":
            ...
            for i in range(len(ft_counts)):
                if ft_counts[i] > 0:
                    ...
                    # Otherwise, continue
                    # Note that this can still reach and end state if the card drawn
...
class QLearningAlgorithm(util.RLAlgorithm):
    ...
    def incorporateFeedback(self, state, action, reward, newState):
        ...
        # Compute the current estimate of Q_opt
        Q_current = self.getQ(state, action)
        # Compute the step size
        stepSize = self.getStepSize()
        # Compute V_opt(s'), which is the maximum Q obtained from actions taken at the newState
        # i.e. max(Q(s',a')) for actions at s'
        Q_opts = []
        for action in self.actions(newState):
            Q_opts.append(self.getQ(newState, action))
        V_opt = max(Q_opts)

        # Compute the features used to update the weights (dict)
        features = self.featureExtractor(state, action)

        # Perform the update
        target_term = reward + (self.discount*V_opt)
        for feature in features:
            key = feature[0]
            value = feature[1]
            self.weights[key] = self.weights.get(key, 0) - (stepSize*(Q_current - target_term)*value)
...
def blackjackFeatureExtractor(state, action):
    ...
    features = []
    # Indicator on action and total cards in hand
    features.append((action, total))

    # Indicator on the action and presence / absence of face values in the deck
    if counts != None:
        count_ind = [0 for elem in counts]
        for i in range(len(counts)):
            if counts[i] > 0: count_ind[i] = 1
            else: count_ind[i] = 0
        features.append((action, tuple(count_ind)))

        # Indicators on the action and number of cards remaining 
        for i in range(len(counts)):
            features.append((action, counts[i]))
