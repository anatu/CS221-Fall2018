...
def betterEvaluationFunction(currentGameState):
  ...
  # def find_closest_capsule(pacman_pos, capsule_pos):
  ...
  if len(capsules) > 0:
    nearest_capsule = len(capsules)/min(capsule_dists)
    ...
    if centroid_dist > 0:
      food_cent_dist = len(capsules)/agent_dist(pacman_pos, centroid_pos)
    else:
      food_cent_dist = len(capsules)
      scores.append(food_cent_dist)
  else:
    nearest_capsule = len(capsules)
    scores.append(nearest_capsule)
