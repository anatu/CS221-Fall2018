...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        gridSize = [self.belief.getNumRows(), self.belief.getNumCols()]
        for i in range(gridSize[0]):
            for j in range(gridSize[1]):
                loc_X = util.colToX(j)
                loc_Y = util.rowToY(i)
                # Compute mean of the prob distribution as Euclidean dist (i.e. the real distance)
                mean = math.sqrt((agentX - loc_X)**2 + (agentY - loc_Y)**2)
                # Pull in STDEV from Const class
                stdev = Const.SONAR_STD
                # Calculate the probability of the observed distance
                # given the distance Gaussian (i.e. prob D_t)
                D_t = util.pdf(mean, stdev, observedDist)
                # Update each belief with the new probability by multiplying w/ local conditional
                old_prob = self.belief.getProb(i, j)
                new_prob = self.belief.setProb(i, j, old_prob*D_t)
...
class ParticleFilter(object):
    ...
    def elapseTime(self):
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        # Create dict to store incremented particles
        elapsedParticles = collections.Counter()
        # Iterate through each tile location w/ particles in it
        for key in self.particles.keys():
            # Fetch number of particles in that tile and the
            # transition probability  
            numParticles = self.particles[key]
            # Pull the range of possible transition probabilities
            # (since key is just the source tile, tp is a dict of probabilities
            # for all possible destination tiles from that source tile)
            tp = self.transProbDict[key]
            # Iterate through each particle in that tile
            for i in range(numParticles):
                # Make a weighted random choice of destination tile
                # based on the transition probabilities tp
                newKey = util.weightedRandomChoice(tp)
                elapsedParticles[newKey] = elapsedParticles.get(newKey, 0) + 1
        # Overwrite self.particles once we have finished building the new dict
