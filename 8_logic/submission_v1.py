...
def ints():
    ...
    1_succNotEqual = And(Successor(x,y),Not(Equals(x,y)))
    1_formula = Equiv(1_succNotEqual, Equals(z,y))
    1_list = [Successor(x,z),Not(Equals(x,z)),Forall(y, 1_formula)]
    1_result = Forall(x, Exists(z,AndList(1_list)))
    formulas.append(1_result)
    # 2. Each number is either odd or even, but not both.
    # Either odd and not even, or even and not odd, for all variables
    2_result = Forall(x,Or(And(Even(x),Not(Odd(x))),And(Odd(x),Not(Even(x)))))
    formulas.append(2_result)
    # 3. The successor of an even number is odd.    
    # Even variable with successor
    3_evenSuccessor = And(Even(x),Successor(x,y))
    # Implication that the successor is odd
    3_result = Forall(x, Forall(y, Implies(3_evenSuccessor,Odd(y))))
    formulas.append(3_result)
    # 4. The successor of an odd number is even.
    # Odd variable with successor
    4_oddSuccessor = And(Odd(x),Successor(x,y))
    # Implication that the successor is even
    4_result = Forall(x, Forall(y, Implies(4_oddSuccessor,Even(y))))
    formulas.append(4_result)
    # 5. For every number x, the successor of x is larger than x.
    # Generic successor
    5_successor = Successor(x,y)
    # Successor is larger
    5_result = Forall(x, Forall(y,Implies(5_successor,Larger(y, x))))
    formulas.append(5_result)
    # 6. Larger is a transitive property: if x is larger than y and y is larger than z, then x is larger than z.
    6_xBiggerThanY = Larger(x,y)
    6_yBiggerThanZ = Larger(x,z)
    6_xBiggerThanYZ = And(6_xBiggerThanY, 6_yBiggerThanZ) 
    6_result = Forall(x, Forall(y, Forall(z, (Implies(6_xBiggerThanYZ,Larger(x,z))))))
    formulas.append(6_result)
    # END_YOUR_CODE
